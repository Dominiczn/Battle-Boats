...
| 5 | User can place Submarines and rotate with R | Place submarine, press R | Submarine occupies 2 cells and rotation changes orientation | Submarine placed correctly and rotation worked |
| 6 | User can place Carrier and rotate with R | Place carrier, press R | Carrier occupies 3 cells and rotation changes orientation | Carrier placed correctly with valid rotation |
...
| 9 | User selects target using highlighted cursor | Arrow keys + Enter | Cursor highlights cell and Enter fires | Cursor selection worked without text input |
| 10 | Miss feedback updates target tracker | Shoot empty cell | Target tracker shows `M` | Miss correctly marked with `M` |
| 11 | Hit and sunk feedback updates target tracker | Sink a ship | Hits show `H` and sunk ship shows `#` | Hits and sunk ships displayed correctly |
